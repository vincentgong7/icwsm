...
#     area = cell_b1b2_extend(cell, max_length)
    ...
    if timeing == 'before':
        rows = mf3v2_crawl_user(platform, area, start_ts - prior_ts, start_ts, 'order by ts desc')
    else: # timeing == 'after'
        rows = mf3v2_crawl_user(platform, area, end_ts, end_ts + prior_ts, 'order by ts')
...
# # for the mf2p5v2_v2, dedicated for left, right and radiation direction crawl
def mf3v2_mf_np_users_direction(platform, timeing, direction,cell_id, start_ts, end_ts, prior_ts, extra_ts, max_length, norm_dis_scale):
    ...
    if timeing == 'before':
        ...
        rows = mf3v2_crawl_user(platform, area, end_ts, end_ts + extra_ts, 'order by ts')
...
def mf3v2_crawl_user(platform, cell_coord_list, start_ts, end_ts, orderby_c):
...
def mf3v2_crawl_user_content(platform, cell_coord_list, start_ts, end_ts, orderby_c):
...
def mf3v2_crawl_general(cell_coord_list, start_ts, end_ts, select_c, from_c, where_c, groupby_c, orderby_c):
...
def call_membership_fun3_v2(cell_id, record_start_ts, record_end_ts):
...
#     record_end_ts = date2ts('2015-08-22 23:59:59')
    ...
    max_length = (1.36+3*0.19) * prior_ts # (mean+3SD)*Delta_t
    # scale = 0.19 * prior_ts 
...
#         mf2p5v2_mf_np_users_direction(platform, timeing, direction,cell_id, start_ts, end_ts, prior_ts, extra_ts, max_length, norm_dis_scale)
                #         direction: left, right, bidirection, radiation 
        twitter_before = mf3v2_mf_np_users_direction('twitter', 'before','bidirection',cell_id, start_ts, end_ts, prior_ts, extra_ts, max_length, max_length/3)
        twitter_after = mf3v2_mf_np_users_direction('twitter','before', 'bidirection',cell_id,start_ts, end_ts, prior_ts, extra_ts, max_length, max_length/3)
        
        inst_before = mf3v2_mf_np_users_direction('inst','after','bidirection',cell_id, start_ts, end_ts, prior_ts, extra_ts, max_length, max_length/3)
        inst_after = mf3v2_mf_np_users_direction('inst','after','bidirection',cell_id, start_ts, end_ts, prior_ts, extra_ts, max_length, max_length/3)
...
#     cell_id = 1 # 0-3 = cell 1-4
    ...
    run = 'mf3'
    output_file = './{}_cell_{}.txt'.format(run, cell_id)

    result_list = call_membership_fun3_v2(cell_id, record_start_ts, record_end_ts)
    mywritelines2file(mf_result_list_transfer(result_list), output_file)
