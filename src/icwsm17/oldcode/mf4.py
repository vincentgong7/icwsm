...
# mf2.5: membership function based on normalisation distribution
...
# mf2.5v2: membership function based on normalisation distribution
...
# mf4 called it.
# for the mf3_v2: speed distribution from reference paper
# mf3: membership function based on speed distribution from reference paper
...
def mf2p5v2_crawl_user_content(platform, cell_coord_list, start_ts, end_ts, orderby_c):
...
def call_membership_fun2p5_v2(cell_id, record_start_ts, record_end_ts):
...
#     result_list = call_membership_fun2p5_v2(cell_id, record_start_ts, record_end_ts)
#     mywritelines2file(mf_result_list_transfer(result_list), output_file)
...
def call_membership_fun3_v2(cell_id, record_start_ts, record_end_ts):
    print('Start membership function 4.')
...
#         mf3v2_mf_np_users(platform, timeing, cell_id, start_ts, end_ts, prior_ts, extra_ts, max_length)
        twitter_before = mf3v2_mf_np_users('twitter', 'before', cell_id, start_ts, end_ts, prior_ts, extra_ts, max_length)
        twitter_after = mf3v2_mf_np_users('twitter','before', cell_id, start_ts, end_ts, prior_ts, extra_ts, max_length)
        inst_before = mf3v2_mf_np_users('inst','after', cell_id, start_ts, end_ts, prior_ts, extra_ts, max_length)
        inst_after = mf3v2_mf_np_users('inst','after', cell_id, start_ts, end_ts, prior_ts, extra_ts, max_length)
        
        twitter_mf_base = membership_fun1_twitter(cell_points_list[cell_id],start_ts,end_ts)[0]
        #twitter_mf2 = membership_fun2_twitter(cell_id, 112, start_ts, end_ts)
        inst_mf_base = membership_fun1_inst(cell_points_list[cell_id],start_ts,end_ts)[0]
       # inst_mf2 = membership_fun2_inst(cell_id, 112, start_ts, end_ts)
        
        twitter = twitter_before + twitter_after + twitter_mf_base
        inst = inst_before + inst_after + inst_mf_base
        
        total = twitter_before + twitter_after + twitter_mf_base + inst_before + inst_after + inst_mf_base
