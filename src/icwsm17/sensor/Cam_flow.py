...
#     output_file = '{}/{}_{}.txt'.format(outputfolder, function_flag,extra_out_put_flag)
    ...
    result_f_df = pd.DataFrame({extra_out_put_flag_F:result_f})
    print(result_f_df)
#     result_f_df.to_csv(output_file, index=0)
    
    '''--------Camera A-------------'''
    extra_out_put_flag_A = 'A'
#     output_file = '{}/{}_{}.txt'.format(outputfolder, function_flag,extra_out_put_flag)
    cam_flow = Cam_flow(port,dbname,cam_table)
    # add where-clouse if needed, camera = 'ZUID-A', 'ZUID-F1', 'ZUID-F2'
    ...
    result_a_df = pd.DataFrame({extra_out_put_flag_A:result_a})
    print(result_a_df)
...
#
    ...
    df_result["cam_all_flow"] = result_f_df[extra_out_put_flag_F] + result_a_df[extra_out_put_flag_A]
    print(df_result)
